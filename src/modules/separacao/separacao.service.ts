...
export class SeparacaoService {
  ...
  async downloadEtiqueta({
    ...
    if (
      ...
      return await this.downloadEtiquetaNaoDetalhado({ numeroConferencia });
    } else {
      return await this.downloadEtiquetaDetalhado({ numeroConferencia });
    }
  }

  async downloadEtiquetaNaoDetalhado({
    numeroConferencia,
  }: NumeroConferenciaFilter): Promise<Buffer | null> {
    const sql = `
    SELECT
  ...
  `;
    ...
    const volumes = rows.map((row, index) => {
      const seqVol = String(index + 1).padStart(2, '0');
  ...
  async downloadEtiquetaDetalhado({
    numeroConferencia,
  }: NumeroConferenciaFilter): Promise<Buffer | null> {
    const sql = `
    SELECT DISTINCT
